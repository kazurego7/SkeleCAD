...
assert.equal(ordinary.handlers.click,undefined,'clicking does not disable a marker');
assert.equal(node('partitionMarkers').children.length,1,'clicking never toggles a marker out of the partition');
...
(async()=>{
  ...
  api.removeNamed('missing');assert.equal(api.hasPendingChanges(),false);
  api.toggleNamed('candidate_close_b');assert.equal(node('partitionMarkers').children.length,3);
  api.toggleNamed('candidate_close_b');assert.equal(node('partitionMarkers').children.length,2);
  api.removeNamed('candidate_close_b');assert.equal(node('partitionMarkers').children.length,1);
  api.end();api.removeNamed('candidate_close_a');assert.equal(node('partitionMarkers').children.length,1,'touch edits cannot run outside partition');
  api.begin();let scale=.5;
  const project=viewer.projectWorld;viewer.projectWorld=center=>({x:100+center[0]*scale,y:100+center[1]*scale,depth:0});
  context.window.SkeleMobile={isActive:()=>true};
  const diameter=()=>{api.render();return parseFloat(node('partitionMarkers').children[0].style.width);};
  const far=diameter();scale=4;const middle=diameter();scale=8;const near=diameter();
  assert.deepEqual([far,middle,near],[8,24,48],'mobile markers scale with their projected world radius instead of a 44px floor');
  context.window.SkeleMobile.isActive=()=>false;scale=.5;assert.equal(diameter(),12,'desktop retains its minimum marker diameter');
  viewer.projectWorld=project;delete context.window.SkeleMobile;
  console.log('PASS: mobile marker diameter follows projected distance while desktop limits stay unchanged');
  console.log('PASS: named touch actions toggle symmetry, delete markers and reject stale or inactive targets');
