...
## スナップショットの端末間共有（2026-09-19）

- スマホの4件をバックアップして共通保存先へ移行。姿勢・視点・画像・順序を両端末で共有し、各レイアウトと操作方法は維持。
- 同じサーバーの `/api/snapshots` を利用し、表示中は4秒ごとと画面復帰時に更新。整理・ドラッグ中の一覧差し替えを抑止し、更新競合時は上書きせず通知する。
- 取り出し用の一時処理は移行後に撤去。アプリには端末優先・初回取り込み処理を残さない。旧ブラウザ保存は変更せず保管。
- 共有データはワークフロー保存先の `.snapshots/current.json`、移行前のバックアップは `.runtime/snapshot-sync-rollout/phone-original.json`。

## Connect起動済みの再送信準備（2026-09-19）

- 実機で読み込み確認・送信確認の自動クローズを確認。検証実行fe196728ebe24c1db7da2032fc758c2cでは自動クローズから新規取り込み・Sendまで通過し、送信中の観測をbusyと判定した。
- 前面化後にマウスをボタン外へ移動して画像照合への影響を除去。前回プレビュー・新しいダイアログの描画途中はクリックせず観測を継続。関連78テストが通過。

- 新しいファイルを開く前に、前回の待機中の読み込み・送信確認をCancelで閉じる。通常画面はそのまま続行する。
- 送信処理中や操作不能の画面は閉じず、待機せずにスマホへ日本語のエラーを返す。
- 閉じる操作はダイアログ範囲・2回の画面照合・操作可能状態を確認し、一度だけ実行。関連76テストが通過。

## Connectの読み込み確認で停止する問題（2026-09-19）

- OCRで5/Sなどを誤読しないよう受け渡し名のランダム部分を判別しやすい英字12文字に変更。送信後の監視時間を45秒から180秒に延長。

- ダイアログの出現中にボタン位置が変わった場合、クリック前の画面確認を最大8回やり直す。ファイル名・ウィンドウの一致と連続する2画面の一致を維持し、入力の再試行は行わない。関連71テストが通過。

- スマホからの準備でConnectが最前面でない場合、検証済みの対象ウィンドウを手前に表示する。
- 表示後もウィンドウ位置・ボタン周辺画像・画面の鮮度を確認し、確認できない場合はクリックしない。Sendの条件は維持する。
- 前面化の成功・失敗・既に前面・Send除外を含むConnect関連68テストが通過。

## スマホ専用スナップショット整理（2026-09-19）

- 通常の一覧はタップで復元、横スワイプでスクロール。長押しで全画面の整理画面を開く。
- 整理画面は2列・縦スクロール。長押し移動と端での自動スクロール、削除、
  キーボードでも使える前後移動を用意。✓でまとめて保存、×・Escapeで破棄する。
- デスクトップのレイアウトと既存ドラッグ操作は従来の処理を継続。

## スマホのスナップショット欄の余白（2026-09-19）

- 一覧の高さを90pxから120pxへ広げ、カードと横スクロールバーの間に
  操作用の余白を確保。縦横画面のモデル表示とステータスの位置も調整。

## スナップショット切替の即時表示（2026-09-19）

- 保存した姿勢をカメラと同じ描画で反映し、前の姿勢の衝突判定を待たない。
- 古い判定結果による姿勢の巻き戻りを防止。新しい姿勢の判定が完了するまで
  衝突表示を消し、確認済みとして扱わない。

## スマホのスナップショット移動（2026-09-19）

- タッチの暗黙的ポインター捕捉が画像ボタンから一覧に移る際の通知で、
  ドラッグが誤って中断される問題を修正。実際のキャンセルと複数指操作では中断を維持。

## 画像からの生成設定と接続修復（2026-09-14）

- 最終選択はユーザー指定の512・30ステップ。20/25ステップと640も比較したが、
  頭部の形状変化やGPUメモリの余裕を考慮し、標準には採用しない。
  生成計算の実測は512/20=50.99秒、512/25=61.30秒、512/30=73.25秒、
  640/20=63.28秒、640/30=82.60秒。モデル読み込みと後処理を含まない。
  512で約9.2 GiB、640で約11.4 GiBのGPU割り当てを記録。
  読み込みスレッドは4、読み込み・推定・表面計算の時間を個別に記録する。
- 新規画像の形状解像度を384から512、生成ステップを50から30へ変更。
  `hierarchical_full`、seed 3407、num_chunks 8000を維持し、guidance 5.0を設定ファイルで明示。
- 同一画像・seedの実測は384/50で生成計算103.34秒、512/50で112.05秒、
  512/30で73.01秒。読み込み込みはそれぞれ152.17秒、158.53秒、144.89秒。
  読み込み時間と同時負荷が異なる単発測定であり、全処理時間の保証ではない。
- ユーザーの最終指定により、生成後の接続修復を有効化。
  最大2 mmの隙間へ半径1.25 mmの丸い接続材を加える。
  元形状と空洞の保持、STL保存後の接続と閉鎖性を検証し、元の形状と履歴を保存。
  512/30でも頭の分離は残るため、解像度だけによる解決とは扱わない。
  対象の2モデルを修復し、既存マーカーで分割を再検証した。
- 左右対称化の際に内部空洞面を反転させないよう修正。
  左右対称モデルの接続材も正確に左右へ反映する。
- 無効な分割マーカーを除いた後、接続グラフも再計算するよう修正。
  表示が2パーツなのに加工側のグラフには5領域が残る不整合を解消した。
  修復済み2モデルのジョイントツリーと、形状保持・空洞・左右対称・
  マーカー除外の回帰検査を実施。標準ビルドのCAD/STL/3MF/干渉/CAEも通過。
- 比較記録は `build/workflow_rerun/connectivity_512*/inference.json`。
  同ディレクトリの `appearance.stl` とpreviewは接続修復の試験品を含むため、
  修復なしの比較には `inference.glb` または `appearance_before_connectivity.stl` を使う。
- 同一画像・seed 3407・384/50の従来`vanilla`方式も比較した。
  生成計算226.36秒、読み込み込み277.10秒。正の体積を持つ外部分離体は
  高速化版と同じ3個で、首の隙間1.131237 mm、背中の隙間0.424021 mmも一致。
  この画像の分離は`hierarchical_full`の導入により生じたものではない。
  詳細は `build/workflow_rerun/connectivity_dense384_50/comparison.json`。

## Bambu Connect経由の印刷とWindows操作の高速化（2026-09-12）

- リモートの確認後に公式Connectへ取り込み、Import・Print・Sendを各1回実行。ローカルのプリント準備は維持。
- 印刷先・材料・オプション・今回のファイルを照合し、Send前の中止と二重送信防止に対応。
- OCRプロセスを実行内で共有し、画像処理と固定待機を短縮。完全一致する新しい撮影結果のみ認識を再利用。
- ユーザー操作で高速化後も動作確認済み。実機送信試験では印刷開始後にユーザーが取り消し、造形完了・品質は未検証。
- 寸法・材料・CAD形状は変更なし。

## リモートの直接印刷経路（2026-09-12）

- ローカルの「プリント準備」とBambu Studio起動は維持。リモートを「プリント開始」に分離。
- 確認画面に印刷先・プレート・時間・材料量を表示し、確認後だけLANへ送信する経路を追加。
- スライスと形状照合、機器待機状態、送信後のデータ照合、一度限りの開始チケットを検査。
- 実機接続にはサーバー側のプリンター設定が必要。未設定時は開始せず、GUI起動へ代替しない。

## 最初の画像表示を軽量化（2026-09-12）

- 画像工程に長辺960pxのプログレッシブJPEGを配信。現行画像で498,245バイトから47,124バイトへ削減し、元PNGは保持。
- 3D変換の待ち合わせを画像・静的ファイルの配信から分離。画像を優先して読み込み、ETagで再転送を抑制。

## 工程・部品間の軽量表示品質を統一（2026-09-12）

- 部品ごとの誤差基準を廃止。元モデルの全体寸法を基準に全工程・全パーツの表示品質を揃える。
- 一律3,000面の下限を廃止し、小さい部品の過剰な細分化を抑制。複雑な部品は表面誤差に応じて面数を増やす。
- 配信・ブラウザキャッシュを共通基準に対応させ、ユーザー依頼でキャッシュ世代を更新。保存データ・印刷用形状は変更なし。

## 1〜2Mbps向けの軽量表示（2026-09-12）

- リモート表示を表面誤差に応じた面数へ簡略化し、頂点共有形式で配信。ローカル表示・CAD・印刷原本は維持。
- 一律の面数で胴体が崩れる問題を修正。双方向の表面距離を確認し、必要な部品だけ細かさを残す。加工済み7部品で約9.5MBから約287KB。
- 可動域は軽量表示を先に操作でき、精密衝突形状を裏で取得。確認完了までは未確認とし、姿勢を引き継ぐ。
- 軽量／精密キャッシュを分離。原本・転送ハッシュ照合、旧サーバー互換、モデル変更時の中断に対応。
- 表示専用の変換であり、寸法・材料・組立は変更なし。CAD/STLトポロジー/組立衝突/CalculiX再ビルドは対象外。

## スマホの配置とタッチ操作（2026-09-12）

- PCの配置を維持し、スマホ専用の縦・横レイアウトを追加。操作説明を画像選択の隣の「？」に集約。
- 分割マーカーは長押しで追加・削除、ダブルタップで左右対称を切り替え。移動・複数指・中断時は長押しを取消。
- 可動域はパーツの直接ドラッグ、背景の視点回転、2本指の移動・拡大縮小に対応。
- 長押しの文字選択・標準メニューを抑止し、入力欄の編集は維持。寸法・材料・CAD変更なし。

- 姿勢カードの削除ボタンを廃止。一覧内は並べ替え、一覧から80px以上離して放すと削除。中断・複数指・モデル変更で取消。
- 工程名を動かさず、ロード中は矢羽根の輪郭を光が周回。警告記号を廃止し、スマホのエラー表示の横幅を修正。

## Tailscale公開先と低速回線での表示（2026-09-12）

- 公開先を `/3dviewer` から `/skelecad` に変更。末尾スラッシュの有無に対応し、
  ビュワー専用だった旧ルートだけ解除。他サービスの公開設定は保持。
- 起動時に確認したTailscaleホストからAPIを利用可能にし、ホスト・オリジン・
  変更操作のセッショントークン確認を維持。接続方式を次回起動にも保存。
- gzip、ETag、SHA-256付きモデルURLの通信キャッシュを追加。
  リモートでの先読みを止め、選択変更時に不要な転送を中断。
- 一覧用JPEGの縮小・遅延取得、非表示の元画像取得の抑制、待機時の状態確認頻度を削減。
- 実際の7部品モデルは22,854,888 bytesから9,670,846 bytes（57.7%減）。
  全モデルのハッシュ完全一致、304応答、Edgeでの初回表示と再表示を確認。
  1Mbps相当の制限で904,941 bytesの1部品を7.33秒で取得。
- 配信サーバー8テストと画面の全テストを通過。寸法・材料・CAD・組立は変更なし。
  CAD/STLトポロジー/組立衝突/CalculiXの再ビルドは対象外。
...
## 2026-09-14: isolated image-generation specks

- Apply cleanup to symmetry outputs as well: historical originals can contain
  specks removed from the current appearance after the original was saved.

- Added physical speck limits: extent 1.5 mm, volume 0.5 mm3, separation over 2 mm.
- Preserve internal cavity shells and nearby anatomy; remove isolated specks even
  when high resolution gives them 100 or more faces. Record removals in manifests.
- Resolution 512, 30 steps, and additive connection repair remain selected.

### 2026-09-19 印刷プロファイルの標準設定
- 通常の印刷準備はサポート有効化だけを上書きし、サポート詳細・強度・速度はBambu標準プロファイルを継承。
- 印刷設定テスト8件成功。現在のモデルの配布用3MFを再生成し、形状一致監査も成功。

### 2026-09-19 画像モデルの関節保持と比較試験片R4
- C4_28のボール6 mm・軸2.8 mm・開口5.6 mmを維持し、画像ワークフローの受け内径を6.15→6.05 mmに調整。独立した加工リビジョンを生成し、従来データを保持。
- R4はT1〜T5の受け内径6.15/6.05/6.00/5.95/5.925 mm。5組10個、点数で識別、標準印刷設定のA1 mini用3MFを生成。
- CAD/STL/3MF、12方向30度以上の軸可動、抜け止め障壁、プレート非接触を検証。5軸の1 N横荷重CalculiXを完了。締まりと保持トルクは実物比較が必要。
- 全体build.ps1完了：CAD、STLトポロジー、アセンブリ衝突、CalculiX（等価断面・実部品）成功。assembly.png再生成済み。画像モデルの新リビジョンも21組の干渉なし。

### 2026-09-19 締まり比較R5（前回5番以上）
- 前回R4 T5が最適だがさらに締めたいという実物評価を記録。U1はT5同等、追加要望によりU1〜U10の10段階へ拡張。内径5.925〜5.700 mmを0.025 mm刻みで比較。
- 6 mmボール・2.8 mm軸・5.6 mm開口を維持。刻印1〜10付きの10組20個の独立したR5試験片として生成し、本体の設定は変更しない。
- 強い締め代と開口のアンダーカットを分離して検証。R5では受け内径の参照球の着座・引き抜き重なりを検査し、実ボールの重なりを併記。既存R2〜R4の判定は維持。開口のない半球カップを誤合格させない回帰テストを追加。
- 最終10段階のCAD/STL/3MF・可動検証成功（12方向で最小36度）。20個のプレート干渉なし。刻印10の両文字を画像と3件の回帰テストで確認。全体build.ps1成功、刻印修正後の10軸のCalculiX解析も再完了。

### 2026-09-19 R5 U8の実物評価を画像モデルへ反映
- ユーザー評価「8はぎりぎり入ってちょうどよい、9は入らない」を保存。画像モデルの標準をU8、受け内径5.750 mm（直径締め代0.250 mm）に変更。6 mmボール・2.8 mm軸・5.6 mm開口は維持。
- 負のクリアランスは実物で選択した試験片と寸法が一致する場合のみ採用し、未選択のきつい寸法への変更を拒否。本体の保持・疲労の保証とは区別する。
- 全体build.ps1成功（CAD・STL/3MF・アセンブリ干渉・CalculiX・assembly.png再生成）。現在の画像モデルを独立リビジョン75ff6c903b9c492f942271db649fd9d9として再加工し、6関節のU8設定、30 CAD工具の妥当性、21組の不要干渉なしを確認。選択寸法と不正な変更拒否の回帰テスト3件成功。
