...
& $python (Join-Path $project 'tools/prepare_retention_print.py')
if ($LASTEXITCODE -ne 0) { throw 'Trial print preparation failed' }
$out=Join-Path (Join-Path $project $parameters.joint_retention_trial.output_directory) 'print'
$inputFile=Join-Path $out 'input.3mf'
$argsList=@('--slice','1','--arrange','0','--orient','0','--outputdir',('"'+$out+'"'),
            '--export-3mf','SkeleCAD_Retention_R1_A1mini_PLA_Matte.3mf',('"'+$inputFile+'"'))
...
& $python (Join-Path $project 'tools/audit_retention_print.py')
if ($LASTEXITCODE -ne 0) { throw 'Retention print release audit failed' }
Write-Host (Join-Path $out 'SkeleCAD_Retention_R1_A1mini_PLA_Matte.3mf')
